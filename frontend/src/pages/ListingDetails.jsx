...
const ListingDetails = () => {
  ...
  return (
    <div className="pb-36 md:pb-32 bg-white">
      {/* Mobile Top Bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-16 px-4 sm:px-6 flex justify-between items-center z-50 bg-gradient-to-b from-black/50 to-transparent pointer-events-none">
      ...
      {/* Image Section */}
      <div className="mx-auto max-w-5xl px-4 sm:px-6 pt-20 md:pt-28">
        <div className="relative h-64 sm:h-72 w-full overflow-hidden rounded-[1.5rem] sm:rounded-[2rem] bg-gray-100 shadow-xl shadow-gray-100 md:h-[420px]">
          <img 
            src={listing.images[0] || 'https://via.placeholder.com/1200x800'} 
            className="h-full w-full object-cover" 
            alt={listing.title} 
          />
          <div className="absolute bottom-4 left-4 sm:bottom-5 sm:left-5 bg-white/90 backdrop-blur px-3 sm:px-4 py-2 rounded-2xl font-black text-slate-950 shadow-lg text-base sm:text-lg">
        ...
        <div className="flex flex-col md:flex-row justify-between gap-8">
          <div className="flex-1 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <h1 className="text-2xl sm:text-3xl md:text-4xl font-black text-slate-950 leading-tight">{listing.title}</h1>
            ...
            {/* Landlord Info */}
            <div className="bg-slate-50 p-4 sm:p-6 rounded-[1.5rem] sm:rounded-[2rem] border border-gray-100 flex items-center gap-4">
      ...
      {/* Mobile Sticky CTA */}
      <div className="md:hidden fixed bottom-20 left-0 right-0 p-4 sm:p-6 bg-gradient-to-t from-white via-white/90 to-transparent flex gap-3 z-40">
        {!isOwner && (!user || user?.role === 'student') && (
          <>
            <button onClick={handleContact} className="flex-1 bg-slate-950 text-white font-black py-4 sm:py-5 rounded-[1.25rem] sm:rounded-[1.5rem] flex items-center justify-center gap-2 shadow-xl shadow-slate-100 text-sm sm:text-base">
              <MessageSquare size={20} /> Message
            </button>
            <button 
              onClick={() => document.querySelector('#visit-form')?.scrollIntoView({ behavior: 'smooth' })} 
              className="flex-[1.5] bg-emerald-600 text-white font-black py-4 sm:py-5 rounded-[1.25rem] sm:rounded-[1.5rem] flex items-center justify-center gap-2 shadow-xl shadow-emerald-100 text-sm sm:text-base"
            >
      ...
      {(!user || user?.role === 'student') && !isOwner && (
        <section id="visit-form" className="md:hidden px-4 sm:px-6 py-10 bg-gray-50 mt-10">
          <div className="bg-white border border-gray-100 shadow-xl rounded-[2rem] sm:rounded-[2.5rem] p-5 sm:p-8 space-y-6">
